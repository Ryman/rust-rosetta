...
fn expand_item(item: &str, result: &mut Vec<int>) {
    // Handle the case of a single number
    for cap in regex!(r"^(-?\d+)$").captures_iter(item) {
        return result.push(from_str(cap.at(0)).unwrap());
    }
    ...
    for cap in regex!(r"^(-?\d+)-(-?\d+)$").captures_iter(item) {
        ...
        return result.extend(range_inclusive(left, right));
    }
...
fn test_basic() {
    let range = "1-5,6";
    assert_eq!(expand_range(range).as_slice(), &[1, 2, 3, 4, 5, 6]);
